...
/**
 * Servlet implementation class Modifierelement
 */
@WebServlet("/Modifierelement")
public class Modifierelement extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int c2=0;
		try {
			c2=cl.Updateelement(nomelement,dated,datef,etat,partn1,mateq1);
		 ...
		 if(c2!=0) { 
			 request.getRequestDispatcher("elementsucceupdate.jsp").forward(request, response);
			
			
		 }
		 else {
			 request.getRequestDispatcher("elementfieldupdate.jsp").forward(request, response);
